...
func initialiser_logos(logos):
	#print("initialisaation minimorpion")
	for case in minimorp:
		case.initialiser_minimorpion(logos)
...
func finish():
	var icon_winner = minimorp[0].case[win]
	if (win == 0):
		icon_winner = minimorp[0].case[3]
	for ch in get_children():
		remove_child(ch)
	#var button_big_case = Button.new()
	var big_case = TextureRect.new()
	#push_warning(win)
	big_case.set_texture(icon_winner)
	
	columns = 1
	big_case.expand_mode = TextureRect.EXPAND_FIT_HEIGHT
	#big_case.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	#big_case.size_flags_vertical = Control.SIZE_EXPAND_FILL
	
	add_child(big_case)
	#button_big_case
	#add_child(button_big_case)
	get_child(0).size_flags_horizontal = Control.SIZE_EXPAND_FILL
	get_child(0).size_flags_vertical = Control.SIZE_EXPAND_FILL
	big_case.custom_minimum_size = Vector2(308,308)
	#big_case.set_size(Vector2(10,10));
